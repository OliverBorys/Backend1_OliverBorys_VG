...
app.get("/api/products/:id", (req, res) => {
  const row = db.prepare(`
    ...
    WHERE p.id = ?
  `).get(req.params.id);
  if (!row) return res.status(404).json({ error: "The product does not exist" });
  res.json(row);
});
...
app.post("/api/products", requireAdmin, (req, res) => {
  ...
  try {
    ...
    res.status(400).json({ error: "Could not save product", details: String(e) });
  }
});
...
app.put("/api/products/:id", requireAdmin, (req, res) => {
  ...
  if (!result.changes) return res.status(404).json({ error: "The product does not exist" });
...
app.put("/api/hero-images/:id", requireAdmin, (req, res) => {
  ...
  if (!result.changes) return res.status(404).json({ error: "Could not find hero-image" });
  ...
  try {
    const tx = db.transaction((userId, items) => {
      ...
      for (const it of items) {
        ...
        if (!it.productId || Number.isNaN(qty) || qty <= 0) throw new Error("Invalid orderline");
...
app.get("/api/orders/:id", requireAuth, (req, res) => {
  ...
  if (!order) return res.status(404).json({ error: "The order does not exist" });
